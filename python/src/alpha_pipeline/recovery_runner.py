...
# ──────────────────────────────────────────────────────────
# 0) Configuración de Rutas y Entorno (Sincronizado con runner.py)
# ──────────────────────────────────────────────────────────
...
# FIJAR EL DIRECTORIO DE LA EJECUCIÓN ESPECÍFICA PARA RECUPERACIÓN
TARGET_EXEC_NAME = "exec_20260313_215724"
CURRENT_EXECUTION_DIR = EXPERIMENT_DIR / TARGET_EXEC_NAME
...
RECOVERY_STORY_START = "g28-zooniverse"

if not CURRENT_EXECUTION_DIR.exists():
    print(f"ERROR: No se encontró el directorio {CURRENT_EXECUTION_DIR}")
    sys.exit(1)
...
# ──────────────────────────────────────────────────────────
# 2) Parámetros Evolutivos (Mismos que runner.py)
# ──────────────────────────────────────────────────────────
POP_SIZE        = 10
GENERATIONS     = 5
ELITE           = 5
...
# ──────────────────────────────────────────────────────────
# 9) Main (MODIFICADO PARA RECUPERACIÓN)
# ──────────────────────────────────────────────────────────
def main():
    ...
    print(f"🔄 Iniciando RECUPERACIÓN en: {CURRENT_EXECUTION_DIR}")
    
    # 1. Identificar historias a procesar (g21 en adelante)
    all_datasets = sorted(USER_STORIES_DIR.glob("*.txt"))
    recovery_datasets = [txt for txt in all_datasets if txt.stem >= RECOVERY_STORY_START]
    
    if not recovery_datasets:
        print(f"No se encontraron historias que empiecen por {RECOVERY_STORY_START}")
        return

    print(f"Historias a procesar: {[d.stem for d in recovery_datasets]}")

    # 2. Cargar resultados existentes para no perderlos
    existing_results = []
    if CSV_MASTER_PATH.exists():
        print(f"Cargando resultados previos de {CSV_MASTER_PATH}")
        df_old = pd.read_csv(CSV_MASTER_PATH)
        # Filtrar para eliminar SOLO los registros que vamos a re-ejecutar (misma historia Y mismo modelo)
        # Esto preserva los resultados de GPT-5.1 si solo re-ejecutamos Gemini.
        df_old = df_old[~((df_old['story'].isin([d.stem for d in recovery_datasets])) & (df_old['model'].isin(MODELS)))]
        existing_results = df_old.to_dict('records')
    
    all_results = existing_results
    
    for txt in recovery_datasets:
        dataset_id = txt.stem
    ...
    if all_results:
        ...
        print(f"📊 Exportado CSV global actualizado a: {CSV_MASTER_PATH}")
